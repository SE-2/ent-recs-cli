...
class DioHttpClient implements IHttpClient {
  ...
  DioHttpClient({required String baseUrl, Duration? timeout})
      : _dio = Dio(BaseOptions(baseUrl: baseUrl, connectTimeout: timeout)) {
    _dio.interceptors.add(LogInterceptor(requestBody: true, responseBody: true));
  }

  @override
  Future<HttpResponse> get(HttpRequest request) async {
    final response = await _dio.get(request.url, options: Options(headers: request.headers));
    return HttpResponse(response.statusCode ?? -1, response.headers.map, response.data);
  }
